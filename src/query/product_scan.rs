...
mod tests {
    ...
    fn product_test() {
        ...
        let mut ts2 = TableScan::new(Rc::clone(&tx1), "T2", Arc::clone(&layout2)).unwrap();
        for i in 1..=5 {
            ts2.insert().unwrap();
        ...
        while ps.next().unwrap() {
            let a = ps.get_int("A").unwrap();
            let b = ps.get_string("B").unwrap();
            let c = ps.get_int("C").unwrap();
            let d = ps.get_string("D").unwrap();
            assert_eq!(a, i / 5 + 1);
