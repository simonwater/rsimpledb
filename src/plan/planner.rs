use std::sync::Arc;
use crate::parse::{Parser, QueryData, UpdateCommand, CreateCommand};
...
impl Planner {
    ...
    pub fn create_query_plan(&self, qry: &str, tx: Arc<std::sync::Mutex<Transaction>>) -> Box<dyn Plan> {
    ...
    pub fn execute_update(&self, cmd: &str, tx: Arc<std::sync::Mutex<Transaction>>) -> i32 {
        let mut parser = Parser::new(cmd);
        let data = parser.update_cmd();
        self.verify_update(&data);
        
        match data {
            UpdateCommand::Insert(insert_data) => {
                self.uplanner.execute_insert(&insert_data, tx)
            }
            UpdateCommand::Delete(delete_data) => {
                self.uplanner.execute_delete(&delete_data, tx)
            }
            UpdateCommand::Modify(modify_data) => {
                self.uplanner.execute_modify(&modify_data, tx)
            }
            UpdateCommand::Create(create_cmd) => {
                match create_cmd {
